...
def compute_CTC_SEG_memory_friendly(gt_mask, pred_mask):
    """
    Memory-efficient SEG computation for large 3D volumes.
    ...
    # Count overlaps using a sparse dict approach instead of huge sparse matrix
    overlaps = Counter(zip(gt_flat, pred_flat))

    # Precompute voxel counts per label
    gt_sizes = {label: np.sum(gt_flat == label) for label in gt_labels}
    pred_sizes = {label: np.sum(pred_flat == label) for label in pred_labels}
    ...
    for gt_id in gt_labels:
        ...
        # Extract all overlaps for this GT label, ignoring background pred=0
        label_overlaps = {pred_id: count for (g, pred_id), count in overlaps.items()
                          if g == gt_id and pred_id != 0}
        if not label_overlaps:
            per_object_J[gt_id] = 0.0
            continue

        # Apply >0.5*|R| criterion
        valid = {pid: cnt for pid, cnt in label_overlaps.items() if cnt > 0.5 * gt_size}
        if not valid:
            per_object_J[gt_id] = 0.0
            continue

        # Take the first valid prediction
        pred_id, inter = next(iter(valid.items()))
