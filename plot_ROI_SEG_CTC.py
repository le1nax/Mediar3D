...
models = {
    "Mediar + Paper Weights": {
        "path": "/work/scratch/geiger/Datasets/CTC/Fluo-N3DL-DRO/02_final_test_res/paper/summary_boxplot/summary_seg_boxplot_iou_mean_paper1.csv",
        "threshold": 0.0001,
    },
    "Mediar + SAM2 Encoder": {
        "path":  "/work/scratch/geiger/Datasets/CTC/Fluo-N3DL-DRO/02_final_test_res/hiera_morethresh/summary_boxplot/summary_seg_boxplot_iou_mean_hiera2.csv",
        "threshold": 0.01,
    },
    "Mediar + Extended Pretraining": {
        "path": "/work/scratch/geiger/Datasets/CTC/Fluo-N3DL-DRO/02_final_test_res/condor/summary_boxplot/summary_seg_boxplot_iou_mean_condor1.csv",
        "threshold": 0.0001,
    }
}
...
TITLE_FONTSIZE = 20
BAR_LINEWIDTH = 2.0
MEDIAN_LINEWIDTH = 3.5  # Thicker median line
QMARKER_SIZE = 6
...
for (model_name, model_info), color in zip(models.items(), colors):
    ...
    for _, row in df.iterrows():
        ...
        ax.plot([x, x], [row['Lower_SEG'], row['Highest_SEG']],
                color=color, alpha=0.15, linewidth=BAR_LINEWIDTH+0.5)

        # Q1–Q3 range (slightly stronger)
        ax.plot([x, x], [row['SEG_Q1'], row['SEG_Q3']],
                color=color, alpha=0.25, linewidth=BAR_LINEWIDTH + 1.5)

        # Quartile + min/max markers (faint)
        ax.scatter(x, row['Lower_SEG'], color=color, marker='_', s=QMARKER_SIZE * 4, alpha=0.7)
        ax.scatter(x, row['SEG_Q1'], color=color, marker='_', s=QMARKER_SIZE * 4, alpha=0.7)
        ax.scatter(x, row['SEG_Q3'], color=color, marker='_', s=QMARKER_SIZE * 4, alpha=0.7)
        ax.scatter(x, row['Highest_SEG'], color=color, marker='_', s=QMARKER_SIZE * 4, alpha=0.7)
